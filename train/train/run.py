...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    # scores
    scores = model.evaluate(dataset.get_test_sequence(), verbose=train_conf["verbose"])
    ...
    logger.info(f"Training parameters saved to {artefacts_path}/params.json")
    ...
    logger.info(f"Label index saved to {artefacts_path}/labels_index.json")
